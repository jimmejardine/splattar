...
- VO segmentation (`solve_segments`): one anchor only connects the keyframes reachable through continuous tracks — the solve recurses into unsolved flanks, each segment its own gauge with landmark state reset between (gauge leaks otherwise). PnP requires ≥12 landmark support: without it, a handful of spurious fb-passing tracks glue poses across genuine continuity breaks and poison the gauge (the forward-backward check is defeated by self-consistent false matches on repetitive texture — seen on synthetic wallpaper patterns). `run` builds submap-0 from the largest segment and pushes the rest through the same Sim(3) register-or-island path as another video; ranges that can't bootstrap (low-parallax footage) are dropped honestly, not forced.
