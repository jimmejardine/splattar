...
cargo run -p gs-cli -- view <ply|project>  # render a splat file, or compose a project dir (components laid out along +x)
cargo run -p gs-cli -- add <video.mp4> [--project <dir>]  # ingest a video (default project: gs-project next to the video); the ONLY ingestion command — add order doesn't matter
...
- **No submap has a privileged gauge.** Projects (default dir: `gs-project` next to the first-added video) store only pairwise Sim(3) edges (`edge=` lines in meta.txt: this submap's coords → target submap's coords; no edges = island); there is no absolute "project world" and no identity-gauge marker. Placement is resolved at compose/register time by union-find + BFS from each component's lowest-index submap (`project::resolve_placements`) — connectivity is add-order-independent; only submap indices and the +x component layout follow add order (presentation-only). Cross-component registration must group candidate matches per component before RANSAC — pooling across components mixes unrelated gauges into one model.
...
- VO segmentation (`solve_segments`): one anchor only connects the keyframes reachable through continuous tracks — the solve recurses into unsolved flanks, each segment its own gauge with landmark state reset between (gauge leaks otherwise). PnP requires ≥12 landmark support: without it, a handful of spurious fb-passing tracks glue poses across genuine continuity breaks and poison the gauge (the forward-backward check is defeated by self-consistent false matches on repetitive texture — seen on synthetic wallpaper patterns). `add` pushes every solved segment through the same Sim(3) register-or-island path; ranges that can't bootstrap (low-parallax footage) are dropped honestly, not forced.
...
- Geometry losses: `examples/geo_bench` measured them at ~zero marginal cost per iteration, and `gs-cli add` enables them at `geo_start: 1500` (the earlier ">10× per iteration" observation tracked scene evolution, not these kernels). The trainer now logs per-phase host ms and sampled per-kernel GPU ms every `log_every` — trust those numbers over folklore when perf shifts.
