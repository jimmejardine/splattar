...
- Submaps are built anchor-out: the causal pass (decode → KLT tracks → sharpness scores) runs once in decode order; geometry (bootstrap, spline, mapping) starts at the segment's best-conditioned anchor window and grows in both temporal directions. Don't conflate decode order with estimation order, and never bootstrap at a segment boundary — boundaries sit at low-quality footage by construction.
