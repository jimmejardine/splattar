...
- **Video decode:** NihAV software H.264 default; **image-folder input as first-class fallback from day one**; document "record H.264/Most Compatible"; Vulkan Video NVDEC (via `ash`) as stretch for HEVC.
...
- **M6 — Video ingest:** demux, NihAV decode, color-correct YUV→RGB (golden frames), keyframe selector.
...
| iPhone HEVC not decodable in pure Rust | image-folder fallback day one; H.264 capture guidance; Vulkan Video in M10 |
