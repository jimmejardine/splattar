...
- **Supervision is spaced by viewpoint, and model size follows the submap.** Training views are the sharpest keyframe per `--view-spacing` degrees of *apparent view change* (rotation plus translation-over-median-scene-depth, one currency), not per 0.25 s of elapsed time — time windows spend the view budget where the operator lingered rather than where the scene is, which left whole stretches of a 1749-keyframe walkthrough near-unsupervised while a doorway got dozens of near-identical frames. The surfel budget follows the resulting view count (`SURFELS_PER_VIEW`, calibrated so the historical 120 views still yields 150k) instead of being flat, so a walkthrough and a 30-keyframe fragment no longer get the same model. `--max-views`/`--budget` remain as explicit overrides; the only hard ceiling left is `VIEW_ATLAS_CAP`, which exists because every view's target is GPU-resident for the whole run, and it logs when it bites.
